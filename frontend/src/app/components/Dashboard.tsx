...
import { parseTaskDate, isTaskOnDay } from '@/app/utils/dateUtils';
...
export function Dashboard({
  ...
  const sanitizedTasks = useMemo(() => {
    return tasks.map(t => {
      if (t.isSpecial || !t.description || !t.description.includes('<!-- metadata:')) return t;
  ...
  // Unified date parsing helper moved to utils/dateUtils.ts
  ...
  const getCoordinates = (e: React.MouseEvent | React.TouchEvent | MouseEvent | TouchEvent) => {
    if ('touches' in e && (e as React.TouchEvent).touches.length > 0) {
      return { clientX: (e as React.TouchEvent).touches[0].clientX, clientY: (e as React.TouchEvent).touches[0].clientY };
  ...
  const eventsForGrid = useMemo(() => {
    ...
    sanitizedTasks.forEach(task => {
      // Find matching day using unified utility
      const dayMatch = weekDays.find(d => isTaskOnDay(task.date, d));
      if (dayMatch) {
  ...
  return (
    <div className="flex h-[100dvh] bg-gray-50 dark:bg-[#1f1f1f] text-gray-900 dark:text-gray-100 overflow-hidden font-sans selection:bg-[#e0b596]/30">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-gray-50 dark:bg-[#1f1f1f]">
        ...
        {/* Header */}
        <header className="min-h-20 md:h-24 pt-[env(safe-area-inset-top)] pb-2 md:py-0 border-b border-gray-200 dark:border-[#292929] flex flex-col justify-center px-4 md:px-6 bg-white dark:bg-[#1f1f1f] sticky top-0 z-40">
          <div className="flex items-center justify-between w-full gap-2">
            <div className="flex items-center gap-3 md:gap-6 min-w-0">
              <button className="lg:hidden text-gray-500 dark:text-gray-400 shrink-0" onClick={() => setShowSidebar(true)}>
                <Menu className="w-6 h-6" />
              </button>

              <div className="flex flex-col min-w-0">
                <h1 className="text-base md:text-xl font-bold text-gray-900 dark:text-white tracking-tight truncate">
                  {new Date().getHours() < 12 ? 'Good Morning' : new Date().getHours() < 18 ? 'Good Afternoon' : 'Good Evening'}, <span className="truncate">{userName}</span>
                </h1>
                <div className={`flex items-center gap-4 ${activeView !== 'calendar' ? 'opacity-0 pointer-events-none' : ''}`}>
                  ...
                  <div className="relative">
                    <button
                      onClick={() => {
                        if (isMobile) setIsMobileCalendarExpanded(!isMobileCalendarExpanded);
                        else setShowMiniCalendar(!showMiniCalendar);
                      }}
                    ...
                    >
                      ...
                      <ChevronDown className={`w-3 h-3 transition-transform ${(isMobile ? isMobileCalendarExpanded : showMiniCalendar) ? 'rotate-180' : ''}`} />
                    </button>
            ...
            <div className="flex items-center gap-2 md:gap-3 shrink-0">
              <div className="relative">
                <button
                  ...
                  className={`flex items-center gap-1.5 md:gap-2 px-2.5 py-1.5 md:px-3 md:py-1.5 border rounded-lg text-xs md:text-sm font-semibold transition-all group ${showFilters || showSpecialsOnly
                        ...
                        <div className="px-2 py-1">
                          ...
                          <div className="grid grid-cols-1 gap-1">
                            {!showViewOptions ? (
                              ...
                              (isMobile ? viewOptions.filter(o => o.id === 'day') : viewOptions).map((option) => (
                                <button
                                  key={option.id}
                                  onClick={() => {
              ...
              <button
                className="flex items-center gap-1.5 md:gap-2 px-3 py-1.5 md:px-4 md:py-1.5 bg-gradient-to-b from-[#e0b596]/90 to-[#c69472]/90 text-[#1f1f1f] shadow-[0_10px_20px_rgba(224,181,150,0.4),inset_0_1px_0_rgba(255,255,255,0.6)] border border-white/20 border-t-white/60 hover:brightness-110 hover:scale-[1.05] backdrop-blur-xl transition-all duration-300 rounded-lg md:rounded-xl text-xs md:text-sm font-semibold whitespace-nowrap"
                onClick={() => setCreateModal({ isOpen: true, duration: 30 })}
              >
                <Plus className="w-4 h-4" />
                <span className="hidden min-[350px]:inline">New</span>
                <span className="hidden sm:inline">&nbsp;Task</span>
              </button>

              <button
                onClick={() => setShowProfileMenu(true)}
                className="h-8 w-8 md:h-10 md:w-10 rounded-full bg-gradient-to-br from-[#e0b596] to-[#dcb49a] flex items-center justify-center text-xs font-bold text-[#1f1f1f] border md:border-2 border-white/50 dark:border-[#333] ml-1 md:ml-2 shadow-sm md:shadow-lg hover:shadow-xl hover:scale-110 transition-all cursor-pointer ring-1 md:ring-2 ring-transparent hover:ring-[#e0b596]/50 shrink-0"
              >
        ...
        <div className="flex-1 relative flex min-h-0 bg-gray-50 dark:bg-[#1f1f1f]">
          <div className={`flex-1 flex flex-col min-w-0 min-h-0 transition-all duration-300 ${showSpecialsOnly ? 'mr-0 lg:mr-64' : ''}`}>
            {activeView === 'calendar' && (
                <div
                  className="flex flex-col h-full bg-white dark:bg-[#1f1f1f]"
                >
                  {isMobile && (
                    <div className="flex flex-col border-b border-gray-100 dark:border-[#333] bg-white dark:bg-[#1f1f1f] shrink-0">
                      {/* View Switcher */}
                      <div className="flex justify-center p-3">
                        <div className="flex bg-gray-100 dark:bg-[#2a2a2a] p-1 rounded-full w-fit shadow-inner ring-1 ring-black/5 dark:ring-white/5">
                          <button
                            onClick={() => setMobileViewMode('schedule')}
                            className={`px-6 py-1.5 rounded-full text-xs font-bold transition-all ${mobileViewMode === 'schedule' ? 'bg-white dark:bg-[#3d3d45] text-[#e0b596] shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                          >
                            Schedule
                          </button>
                          <button
                      ...
                      {/* Persistent Common Datestrip (Paginated or Grid) */}
                        <div className="flex flex-col bg-white dark:bg-[#1f1f1f] shrink-0 relative">
                          {!isMobileCalendarExpanded ? (
                            <div
                              ref={dateStripRef}
                              className="flex overflow-x-auto no-scrollbar py-3 px-0 scroll-smooth snap-x snap-mandatory"
                            >
                                  ...
                                  {continuousCalendarDays.map((day, i) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <span className={`w-9 h-9 flex items-center justify-center rounded-full text-sm font-bold transition-all ${
                                          isSelected 
                  ...
                  {isMobile && mobileViewMode === 'schedule' ? (
                    <div className="flex-1 overflow-y-auto p-4 pb-32 space-y-8 bg-gray-50/30 dark:bg-transparent custom-scrollbar">
                      {(() => {
                        // Only render a small window (7 days) in the vertical list for performance
                        const start = subDays(currentDate, 3);
                        const listDays = Array.from({ length: 7 }).map((_, i) => addDays(start, i));
                                      ...
                                      {dayTasks
                                        ...
                                        .map(task => (
                                          <div
                                            ...
                                            className={`p-4 rounded-2xl border flex items-center gap-4 group transition-all active:scale-[0.98] ${task.completed ? 'bg-gray-50/50 dark:bg-[#252525]/50 border-gray-100 dark:border-gray-800' : 'bg-white dark:bg-[#252525] border-gray-100 dark:border-[#333] shadow-sm'}`}
                                          >
                                            {/* Time & Duration (Left) */}
                                            ...
                                            {/* Colored Bar */}
                                            <div className={`w-1 h-10 rounded-full flex-shrink-0 ${task.completed ? 'bg-gray-300' :
                                              isBefore(setMinutes(setHours(startOfDay(parseTaskDate(task.date)), parseInt(task.time?.split(':')[0] || '0')), parseInt(task.time?.split(':')[1] || '0')), subMinutes(new Date(), 1)) ?
                                                'bg-amber-500' :
                                                task.category?.toLowerCase() === 'work' ? 'bg-blue-500' : task.category?.toLowerCase() === 'personal' ? 'bg-green-500' : 'bg-purple-500'
                                              }`} />
                                            ...
                                            <div className="flex-1 min-w-0">
                                              ...
                                              <p className="text-[10px] md:text-xs text-gray-500 mt-0.5 truncate uppercase">
                  ...
                  {calendarView !== 'month' ? (
                    <div
                      className="flex-1 overflow-y-auto relative custom-scrollbar touch-action-pan-y"
                      ref={containerRef}
                            ...
                            return (
                              <div
                                ...
                                onMouseDown={(e) => {
                                  ...
                                  const dayIdx = getDayIndexFromX(clientX - (containerRef.current?.getBoundingClientRect().left || 0) - 60, (containerRef.current?.getBoundingClientRect().width || 0) - 60, weekDays.length);
                                  setDragStart({
                                    x: 0,
                                    y: 0,
                                    time: event.start,
                                ...
                                onTouchStart={(e) => {
                                  ...
                                  setDragCurrent({ x: 0, y: 0, time: event.start, dayIndex: dayIdx });
                                  setDragMode('move');
                                }}
                                onClick={(e) => {
                                  e.stopPropagation();
                              ...
                              >
                                ...
                                `}
                                >
                                  {!event.completed && !event.isPast && !isMobile && (
                                    <div
                                      ...
                                      onMouseDown={(e) => {
                                        ...
                                        const rect = containerRef.current?.getBoundingClientRect();
                                        const { clientX } = getCoordinates(e);
                                        const dayIdx = rect ? getDayIndexFromX(clientX - rect.left - 60, rect.width - 60, weekDays.length) : 0;
                                        setDragStart({ x: 0, y: 0, time: event.start, dayIndex: dayIdx });
                                        setDragCurrent({ x: 0, y: 0, time: event.end, dayIndex: dayIdx });
                                        setDragMode('resize');
                                      }}
                                      onTouchStart={(e) => { if (isMobile) return; 
                                        e.stopPropagation();
                                        setActiveTaskId(event.id);
                                        const rect = containerRef.current?.getBoundingClientRect();
                                        const { clientX } = getCoordinates(e);
                            ...
                            <div className="flex-1 overflow-y-hidden space-y-1">
                              ...
                              {dayTasks.length > 2 && (
                                <button
                                  ...
                                  className="w-full text-left text-[10px] font-bold text-gray-500 dark:text-gray-400 hover:text-[#e0b596] dark:hover:text-[#e0b596] mt-1 flex items-center transition-colors"
                                >
                                  +{dayTasks.length - 2} more <ChevronRight className="w-3 h-3 ml-px" />
                                </button>
                              )}
                            </div>
                          </div>
                  ...
                  {isMobile && !isSameDay(currentDate, new Date()) && (
                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 pointer-events-none active:pointer-events-none z-50">
                      <button
                        onClick={() => handleDateSelect(new Date())}
                        className="px-6 py-3 bg-white dark:bg-[#292929] border border-gray-200 dark:border-[#333] rounded-full shadow-lg text-[#e0b596] text-sm font-bold flex items-center gap-2 active:scale-95 transition-all pointer-events-auto shadow-[#e0b596]/10"
                      >
                    ...
                    const filteredTasks = sanitizedTasks
                      .filter(t => {
                        if (activeView === 'pending') {
                          ...
                          return isTaskOnDay(t.date, pendingSearchDate);
                        }
                        ...
                        return isTaskOnDay(t.date, completedSearchDate);
                      })
          ...
          <AnimatePresence>
            {showSpecialsOnly && (
              <motion.div
                ...
                className="absolute right-0 top-0 bottom-0 w-64 bg-white dark:bg-[#1b1b1b] border-l border-gray-200 dark:border-[#292929] shadow-2xl z-30 flex flex-col pt-[env(safe-area-inset-top)]"
              >
            ...
            showSidebar && (
              <>
                ...
                <motion.div
                  ...
                  className="fixed inset-y-0 left-0 z-50 w-72 bg-white dark:bg-[#1b1b1b] shadow-2xl p-6 lg:hidden flex flex-col border-r border-gray-200 dark:border-[#292929] pt-[calc(env(safe-area-inset-top)+24px)]"
                >
